...
function VincularResponsavelContent() {
    ...
    const handleVincularResponsavel = async () => {
        if (!selectedResponsavel) {
            toast.error("Selecione um responsável para vincular.", { duration: Number(process.env.NEXT_PUBLIC_TOAST_DURATION) });
            return;
        }
        ...
        try {
            ...
            if (response.status === 201) {
                toast.success("Responsável vinculado com sucesso!", { duration: Number(process.env.NEXT_PUBLIC_TOAST_DURATION) });
                router.push(`/area-cliente/pacientes/responsavel/${dependenteId}`);
            } else if (response.status === 409) {
                // Erro de duplicação (responsável já vinculado)
                const errorData = await response.json();
                toast.error(errorData.erro || "Este responsável já está vinculado a este paciente.", { duration: Number(process.env.NEXT_PUBLIC_TOAST_DURATION) });
            } else {
                const errorData = await response.json();
                toast.error(errorData.erro || 'Erro ao vincular responsável', { duration: Number(process.env.NEXT_PUBLIC_TOAST_DURATION) });
            }

        } catch (error) {
            console.error('Erro ao vincular responsável:', error);
            toast.error(error instanceof Error ? error.message : "Erro ao vincular responsável. Tente novamente.", { duration: Number(process.env.NEXT_PUBLIC_TOAST_DURATION) });
        } finally {
            setVinculando(false);
        }
    };
