        ...
        const fetchDados = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    toast.error('Paciente não encontrado', { duration: Number(process.env.NEXT_PUBLIC_TOAST_DURATION) });
                    router.push("/area-cliente/pacientes");
                }
            } catch (error) {
                console.error('Erro ao buscar dados:', error);
                toast.error('Erro ao carregar dados', { duration: Number(process.env.NEXT_PUBLIC_TOAST_DURATION) });
            } finally {
                setLoading(false);
            }
        };
    ...
    async function efetuaEdicao(data: Inputs) {
        try {
            ...
            if (response.status === 200) {
                toast.success("Paciente editado com sucesso!", { duration: Number(process.env.NEXT_PUBLIC_TOAST_DURATION) });
                setTimeout(() => {
                    router.push("/area-cliente/pacientes");
                }, 2000);
                ...
                toast.error(errorData.erro || "Já existe um paciente cadastrado com este CPF.", { duration: Number(process.env.NEXT_PUBLIC_TOAST_DURATION) });
            } else {
                const errorData = await response.json();
                toast.error(errorData.erro || "Erro ao editar paciente", { duration: Number(process.env.NEXT_PUBLIC_TOAST_DURATION) });
            }
        } catch (error) {
            console.error('Erro ao editar paciente:', error);
            toast.error("Erro ao editar Paciente. Verifique sua conexão.", { duration: Number(process.env.NEXT_PUBLIC_TOAST_DURATION) });
        }
    }
